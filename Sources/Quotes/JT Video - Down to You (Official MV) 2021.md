---
type: quote
citable: true
status: draft   # card text transcribed from Jen's frame screenshots (2026-06-25) — flip to stable after the post URL is pulled + card eye-confirmed verbatim
release_date: 2021   # public release year (the vault's date for this video)
verified_against: ["Jen frame screenshots of the @JagwarTwin 'Down to You (Official Music Video)' — the red end-card + the post UI (2026-06-25)"]
ingested: 2026-06-25
ingest_source: "@JagwarTwin YouTube/IG video 'Down to You (Official Music Video)' (~2021) — Jen frame screenshots, 2026-06-25"
---
# JT Video — *Down to You (Official MV)* (Source)

Verbatim ingest of the @JagwarTwin *Down to You* official video + its on-screen end-card. **No interpretation here** — analysis routes to Brain/Essays. **Third card of the early manifesto-card wave (2019–2021)** (see [[The Manifesto Cards]]); same fixed template as [[Sources/Quotes/JT Video - Happy Face (Official MV) 2020]] and [[Sources/Quotes/JT Video - I Like To Party (Official Audio) 2021]].

## The post (grounded)
- **Account:** **@JagwarTwin**. Title: **"Down to You (Official Music Video)"**.
- **Posted:** ~2021 ("5y ago" at capture, 2026-06-25). Hashtag **#Matter**.
- **Counts at capture (2026-06-25):** ~**3.5K likes** / ~**131K views**.
- **URL:** not yet pulled (Jen screenshots only) — pull the video ID to firm `status: stable`.
- **Note:** *"Down to You"* is also a **Joni Mitchell** song (*Court and Spark*, 1974) — cross-link the Joni thread.

## The card (transcribed from frame — verify by eye)
> **DOWN TO YOU** *(stylized, red)*
> **PRESENTED BY JAGWAR TWIN**
> **WHAT IS IT THAT YOU SEEK?**
> **NONE** *(JT glyph)*  **AS ABOVE SO BELOW**
> *To change the world one must first change the concept of the world. To do that one must change the concept of one's self. The concept one holds of the self makes one see the world as they do.*

### Textual notes (not interpretation)
- **"PRESENTED BY"** matches *I Like To Party*'s "by" frame (vs. *Happy Face*'s *"BROUGHT TO YOU BUY"*).
- The sigil-triad is **"NONE" · JT glyph · "AS ABOVE SO BELOW"**, flanking the seeker-question **"WHAT IS IT THAT YOU SEEK?"**.

## What this grounds (+ routing — folds not yet executed)
- **[G] Manifesto card #3.** → [[The Manifesto Cards]] / → [[The Visual-Canon Throughline (Manifesto Cards, the Realized Circus, the Silent Lucius) 2019-2026]]. Template confirmed, ~2021.
- **[W] → [[The Empty Mirror]] — "WHAT IS IT THAT YOU SEEK? / NONE."** The **seeker-question** answered with **"NONE"** = the **No-Thing** as the answer to *what do you seek* (alongside the Feb-2022 *"no thing ever creates"* tweet). The seeker-line rhymes with WTTC's later seeker-questions.
- **[W] → [[The Pattern]] — "AS ABOVE SO BELOW."** The **Hermetic** axiom (Emerald Tablet) on a public card in **2021** — the 5-year-old root of WTTC's 2026 *"Did Hermes paint his face?"* (the Hermetic thread isn't new).
- **[W] → [[Source vs Reflection]] — the concept-of-self manifesto = the mirror doctrine.** *"To change the world one must first change the concept of the world… the concept one holds of the self makes one see the world as they do"* = change-your-conception-of-self-and-you-change-your-world. **New named-influence lead: Neville Goddard** (almost verbatim — *verify against a primary before grounding*).

## Citation
- @JagwarTwin, *"Down to You (Official Music Video)"* (~2021, #Matter), URL TBD. Captured 2026-06-25 from Jen's frame screenshots (red end-card + post UI).
