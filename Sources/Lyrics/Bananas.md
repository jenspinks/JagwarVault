---
type: lyrics
citable: true
status: stable
verified_against: ["Apple / Shazam + Jen in-hand (2026-06-07): 'the monkeys' (plural); 'light up the stove'; Good/Bad capitalized"]
ingested: 2026-05-25
ingest_source: "Google Drive (Jen's lyrics doc 1 — Lucius Lullaby album)"
---
# Bananas — Lyrics (Source)

Verbatim ingest. **No interpretation here** — analysis lives in Brain/ and Essays/.

## Note
**Roy's banana gloss — PUBLIC, grounded [G] (citation found 2026-06-07):** the banana was chosen because it is a fruit that is **sterile** — flawed genes / inherited trauma carried forward — but **looks healthy and good.** **Exact public source:** @jagwartwin IG comment on the *Bananas* reel — *"most bananas are sterile and cannot reproduce by seed ;)"* ([[Sources/Quotes/Roy IG Reel - Bananas (sterile gloss)]], instagram.com/reels/DWo9DYzAUZ8/); the fuller flawed-genes/looks-healthy gloss is from Jen's firsthand in-person witness of Roy at a public moment. Public-citable (cite the IG comment, not the Drive doc). The sterile reading also stands independently on **public botany** (Cavendish = sterile clone). Separately, a **private phone *call*** that touched the "nonsense" motif stays **Secret-Doc / non-citable** (its call-specific decode is not reproduced here). **CONFLICT RESOLVED (Jen 2026-06-07):** the angle/angel + "nonsense" + mirror framework corresponds to a **documented public Kabbalah** (`[~]` — [[The Angels and the Angles]]: Abulafia / *Sefer Yetzirah* / Ezra ben Solomon); Roy's Hebrew/Kabbalah *engagement* is public `[G]` (2019 Hebrew post; El Roi; "I'm 33" board), but there is **no public source of him stating it as his method.** Canon rests on the public tradition + iconography, not the call.

## Source
- (release metadata pending — on *Lucius Lullaby*, 2026; add Apple/Spotify URLs at verification)

## Lyrics

```
Go Bananas, GoGo Bananas
Go Bananas, GoGo Bananananas

Hey kids!
Do you ever wanna break shit? (break shit)
Do you wanna get wasted?
Yeah, yah!

Well alright (x4)

Go Bananas, GoGo Bananas
Go Bananas, GoGo Banananana (x2) (Yeah)

It's the weekend and I'm freaking, like I'm ready to blow.
Just the monkeys feeling funky, I got nowhere to go
Don't wanna talk, I wanna rock, I wanna light up the stove.

And burn it down, Burn it down, Burn it down down down

Cause I wanna be Good, but it's fun to be Bad,
And I wanna be nice, but I always get mad.
And I wanna be good, but it's fun to be baaahaahaaaahaaahad.

Hey kids! (hey)
Do you ever wanna break shit? (yeah)
Do you wanna get wasted? (uh-huh)

I grew angry, sometimes I can lose my temper. (temper)
I hear voices in my mind I don't remember
[Did I do that?]

Cause I wanna be Good, but it's fun to be Bad (bad bad ba-bad)
And I wanna be nice, but I always get maahaaaahaad.
Oh Maaaan!
```

## Maintenance
- Bulk-ingested 2026-05-25 from Google Drive doc 1 (album doc).
- **Not yet verified** against streaming release. Record URLs in `verified_against`; flip `status` to `stable`. Official release wins on divergence.
