...
referenced_by: [CONCEPT-DividedSelf, CONCEPT-Mozart, CONCEPT-TheTwoBirds]
...
A "Tree of Life" hub is the single most tempting place to flatten distinct streams. **Do not.**
- **Lurianic kelipot ≠ Hermetic/Golden-Dawn Qliphoth** ("the Tree of Death"). The vault's tarot/sefirot/morning-star-Lucifer/Qliphah-of-Keter scaffold (on [[The Light-Bearer]], [[Brain/Characters/Sir Lucius]]) is largely **[[Brain/Concepts/Hermeticism - As Above So Below|Hermetic]]**; the shevirah doctrine is **Jewish** — always label which stream a claim is in.
- **Lurianic ≠ Gnostic.** Luria is **world-affirming** (redeem the material; the break is generative); Roy's public "escape the circus / wake from the fog" frame is **[[Brain/Concepts/Gnosticism - The World as Trap|Gnostic]]** (anti-cosmic). The spark/birur mapping is real *structurally*, never a silent merge of the two teloi. *(The three esoteric-stream hubs — this page = Lurianic · Hermeticism · Gnosticism — all exist & are mutually cross-linked as of 2026-06-29.)*
- Portable doctrine lives at [[Sources/Reference/Shevirat HaKelim (Lurianic doctrine reference)]] §C.

## Two axes of the vault's Kabbalah
The vault engages Kabbalah on **two** axes that *Sefer Yetzirah* itself unites ("22 foundation letters **+** 10 sefirot = 32 paths of wisdom"):
1. **The emanation structure** (this page) — the sefirot and pillars.
...
## The lateral twins — Netzach and Hod as structural equals `[W]` (2026-07-02)
*What Roy's fundamentals do to the **geometry**, not just the labels. Once Roy sits at Netzach and Lucius at Hod, the two stop being vertically ranked (the old picture: Roy the lunar vessel *below*, Lucius *raiding in* from the side) and become what Netzach and Hod actually are on the Tree — **the paired sefirot at the same height**, the two hips flanking the Middle Pillar, both draining into Yesod, both with a direct path up to Tiferet. Neither is above the other. The old asymmetry was an artifact of the old map; Roy's own assignment makes them **mirror twins across the pillars.***

- **"Jagwar Twin" is literalized in the architecture `[W]`.** The artist-name becomes the name of the **Netzach–Hod pair itself** — the two-that-are-one at the base of the Tree. Chimes with Roy's public **Gemini-Sun** self-ID ([[Roy]] §Self-naming, already `[W]`) and tightens [[Jacob & Esau]] (twins of one body, one blessing divided). The project's title was quietly printing the map the whole time.
- **The Middle Pillar is the shared contested axis, not anyone's home `[W]`.** Yesod (moon / image / dream) is the field **both** twins pour into — which is exactly why it can read as Lucius's *theater* and as Roy's lunar *position* at once, without either being wrong. This **reconciles the two dated datapoints** rather than making the newer one overwrite the older: **2026-06-14** ("the Yesod/Lucius mapping is significant") = Lucius *occupying* Yesod, his operating theater; **2026-07-01** ("Hod") = his *seat*. **Seat vs theater — both kept.** A figure works *through* the image-realm; he lives one rung up, on his own pillar.
- **Tiferet stays unoccupied — now structurally guaranteed `[W]`.** Netzach sits directly *below* Tiferet with a path straight up (Nun / "Death" — §path lattice), so *"Roy moves toward the heart-Sun but has not become it"* stops being a hedge and becomes **geometry**: his seat is one path short of the center, by construction, and the inflation-guard is built into the map instead of bolted on. And if any figure ever *names* the integrated Tiferet-state — the twins reconciled at the heart — it is the **meta-figure [[Jagwar Twin]]** (the container, not a voice), never Roy the character. *(Flagged for the Jagwar Twin page.)*
- **The pillars ground the vessel doctrine `[~]`.** Hod sits on the **Pillar of Severity / Form** — Lucius as the **over-contained husk**, structure with no flow. Netzach sits on the **Pillar of Mercy / Force** — Roy as the **over-opened, flood-prone vessel**, flow with no wall (hence *Subject to Flooding*, the debut named for exactly that failure). Two ways a vessel breaks, one per twin, one per pillar — [[Keilim - The Breaking of the Vessels]]'s four-fates material read straight off the base of the Tree.
- **Planetary grace note `[~]`.** Mercury and Venus are the only two **inferior planets** — the two that can never leave the Sun's vicinity, never appear at midnight, only ever haunt the dawn or the dusk. The twins' planets are structurally **sun-bound**: their whole visible existence is *relative to the heart-Sun they flank.* And **Mercury (Lucius) hugs tighter and blinks in and out fastest** — the intellect that never actually gets far from the heart it claims to have left, flickering at the horizon. The astronomy was doing the theology.

## The shoulders, and why the two endings differ `[~]` (Lurianic; added 2026-07-13)
*The [[Jacob & Esau|divided-self array]] and the [[Mozart]] node land on this Tree, and the landing sharpens the geometry above rather than adding to it. Lurianic stream (the Ari's *Sha'ar HaGilgulim* / Gate of Reincarnations, ch. 31); framework `[~]`, never authorial intent.*

- **The shoulders are the upper octave of the two side-pillars.** In the Gate of Reincarnations, **Cain and Abel are the two shoulders of Adam**, Cain on the **left** (Gevurah / severity), Abel on the **right** (Chesed / mercy). That is the same Mercy-vs-Severity split as the **Netzach/Hod lateral twins** one triad down: Chesed is Netzach's upper octave on the Pillar of Mercy, Gevurah is Hod's on the Pillar of Severity. So Roy and Lucius (the hips) are the *lower-world* echo of a fratricide the tradition already stations on the *upper* left and right.
- **Amadeus lands Salieri on Lucius's own pillar.** Read through *Amadeus* (Salieri = Cain, Mozart = Abel; Robbins, JRF 1997, see [[Mozart]]), the craftsman who envies sits on the **left / Severity** (Gevurah → Hod), exactly where Roy's own fundamentals seat **Lucius (Hod)**; the gifted one sits on the **right / Mercy** (Chesed → Netzach), Roy's seat. A convergence, not a coincidence: the outside story and Roy's inside map agree on which side is which.
- **The middle pillar is the whole difference between a mark and an embrace.** Cain/Abel is the pure left-right split with **nothing in the centre to hold it**, so it ends in murder, and *Amadeus* runs the same geometry to the same end. [[Jacob & Esau]] ends instead in **the embrace**, which on the Tree *is* the Middle Pillar: **Tiferet, the heart, holding the two sides.** This is the structural reason the page keeps **Tiferet unoccupied** and reserves the integrated centre for the meta-figure [[Jagwar Twin]]: the un-reconciled prognosis is the two sides with the centre *empty*, the reconciled prognosis is the centre *filled*, and the catalogue is still singing toward the filled centre, not standing on it (`threshold_state: approaching`).
- **Guard.** No Roy statement invokes Cain, Abel, or *Amadeus*; the sole grounded thread is Roy's own *"Mozart's not your homie"* + Lucius post ([[Mozart]]). Everything here is the Lurianic frame read against a divided self Roy built, held as an **alternative script**, one lens among the array's several, never a hidden genealogy.

## The path lattice `[~]` (one rung `[W]`) — analyst extension, flagged
*Roy gave sefirot + planets; he did **not** give the **paths** between them. The 22 connective paths carry standard **Golden Dawn** letter/tarot attributions — centuries-standardized, **not** invented for this fit (that's [[The Pattern]]'s anti-pattern #1 answered in advance: these predate the reading by a hundred years). Hold at `[~]` except the first rung. If Roy's *"it's a kind of code"* (re the Devil card, [[Brain/Characters/Sir Lucius]] §Iconography `[G]`) is systematic, this lattice is the most economical candidate for what it decodes — but that "if" stays an if. Test; don't file above tier.*

- **Hod → Tiferet · the 26th path · Ayin · Trump XV "The Devil" `[W]`.** Ayin means **eye.** Lucius's only road from intellect up to the heart runs through the eye-letter under the Devil card — and Roy pointed at the Devil card + the green apple + the *right eye blocked, left eye open* **independently** ([[Brain/Characters/Sir Lucius]] §Iconography, `[G]`). Four convergent anchors, two of them Roy's own words → **the Devil card as Lucius's blocked road home; the Dweller stationed on his own Threshold.** This rung earns `[W]`.
- **Netzach → Tiferet · Nun · "Death" `[~]`.** Roy reaches the heart only through death-rebirth — *"Life is an act of death"* `[G]`, REBIRTH as a HoM code, [[SOL]]'s white-rose Fool stepping off the ledge. The **sword** (his fundamental) is the Death-path's instrument: the decisive cut.
- **Netzach → Hod · Peh · "The Tower" `[~]`.** Peh means **mouth.** The one path running *between the twins* is the mouth-letter under the lightning-struck tower → **The War** (EP1), Team Roy vs Team Sir Lucius, the Babel⇄*levav* [[Bounce]] reading, THE LIGHTNING FLASH (HoM code). The twins' direct confrontation is Babel; its resolution-letter is speech from the heart.
- **Netzach → Yesod · Tzaddi · "The Star" `[~]`.** Roy enters the image-realm as a star → a title-level hit on [[Soul Is A Star]].
- **Hod → Yesod · Resh · "The Sun" `[~]`.** Lucius enters the image-realm wearing the **Sun's** own trump — *borrowed* solar light, "truth and light divorced from heart" = the **counterfeit sun** → [[Happy Face]] (Lucius-claimed, sun-emblem), the board's broken stage-6 *"mirror of the Soul and the heart."*
...
- **Keter** — the crown that did **not** shatter; the intact upper triad tikkun re-links to ("reclaim your crown" → Keter). *(Structural; not a declared id.)*
